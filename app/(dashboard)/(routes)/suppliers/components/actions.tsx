...
interface ActionsProps {
  customerData: SupplierColumn;
}
const Actions = ({ customerData }: ActionsProps) => {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/suppliers/${customerData.id}`);
    ...
    <>
      ...
      <div className="flex items-center gap-x-2">
        <div
          ...
          onClick={() => {
            router.push(`/suppliers/${customerData.id}`);
          }}
        >
          <FileEdit
            size={10}
