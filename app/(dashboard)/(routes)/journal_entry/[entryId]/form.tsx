  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (initialData) {
        ...
        await axios.patch(`/api/journal_entry/${params.entryId}`, values);
      } else {
        if (values.amount < 0.1) {
        ...
        console.log(values);
        await axios.post("/api/journal_entry", values);
      }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <Card className="dark:bg-transparent">
          <CardHeader>
            ...
            <CardDescription>Journal Entry Creation Form</CardDescription>
          </CardHeader>
          <CardContent>
